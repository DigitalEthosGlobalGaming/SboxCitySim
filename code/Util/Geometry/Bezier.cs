...
namespace GridSystem.Geometry
{
	...
	public class Bezier : IPathShape, IInterval
	{
		...
		private void ComputeDirection()
		{
			...
			float angle = GeometryUtils.Angle( points[0], points[order], points[1] );
		...
		public float Length
		{
			get
			{
				return GeometryUtils.Length( Tangent );
			}
		}
		...
		public List<Vector2> Hull( float t )
		{
			...
			while ( p.Count > 1 )
			{
				...
				{
					Vector2 pt = GeometryUtils.Lerp( t, p[i], p[i + 1] );
					q.Add( pt );
					_p.Add( pt );
				}
				p = _p;
		...
		public Split Split( float t )
		{
		  ...
		  order == 2
			...
			result.Left._t1 = GeometryUtils.Map( 0, 0, 1, _t1, _t2 );
			result.Left._t2 = GeometryUtils.Map( t, 0, 1, _t1, _t2 );
			result.Right._t1 = GeometryUtils.Map( t, 0, 1, _t1, _t2 );
			result.Right._t2 = GeometryUtils.Map( 1, 0, 1, _t1, _t2 );
		...
		public Bezier Split( float t1, float t2 )
		{
			...
			// make sure we bind _t1/_t2 information!
			result.Left._t1 = GeometryUtils.Map( 0, 0, 1, _t1, _t2 );
			result.Left._t2 = GeometryUtils.Map( t1, 0, 1, _t1, _t2 );
			result.Right._t1 = GeometryUtils.Map( t1, 0, 1, _t1, _t2 );
			result.Right._t2 = GeometryUtils.Map( 1, 0, 1, _t1, _t2 );

			// if we have a t2, split again:
			t2 = GeometryUtils.Map( t2, t1, 1, 0, 1 );
		...
		public Extrema Extrema()
		{
			...
			float[] resultx = GeometryUtils.Droots( px.ToArray() );
			float[] resulty = GeometryUtils.Droots( py.ToArray() );
			...
			if ( order == 3 )
			{
				...
				resultx = resultx.Concat( GeometryUtils.Droots( px.ToArray() ) ).ToArray();
				resulty = resulty.Concat( GeometryUtils.Droots( py.ToArray() ) ).ToArray();
			}
			...
			rx.Sort( GeometryUtils.NumberSort );
			ry.Sort( GeometryUtils.NumberSort );
		...
		public BoundingBox BoundingBox
		{
			get
			{
				...
				GeometryUtils.GetMinMaxX( this, extrema.X, out float minx, out float maxx );
				GeometryUtils.GetMinMaxY( this, extrema.Y, out float miny, out float maxy );
		...
		public static bool Overlaps( Bezier curve1, Bezier curve2 )
		{
			...
			return BoundingBox.Intersects( lbbox, tbbox );
		}
		...
		public bool IsSimple()
		{
			if ( order == 3 )
			{
				float a1 = GeometryUtils.Angle( points[0], points[3], points[1] );
				float a2 = GeometryUtils.Angle( points[0], points[3], points[2] );
		...
		public PolyBezier Outline( float d1, float d2 )
		{
			...
			var ls = GeometryUtils.MakeLine( bs, fs );
			var le = GeometryUtils.MakeLine( fe, be );
		...
		public PolyBezier Outline( float d1, float d2, float d3, float d4 )
		{
			...
			Func<float, float> linearDistanceFunction( float s, float e, float tlen0, float alen0, float slen0 )
			{
				return new Func<float, float>( v =>
				{
					...
					var d = e - s;
					return GeometryUtils.Map( v, 0, 1, s + f1 * d, s + f2 * d );
				} );
			}
				...
				while ( t2 <= 1 )
				{
					for ( t2 = t1 + step; t2 <= 1 + step; t2 += step )
					{
						...
						if ( !segment.IsSimple() )
						{
							...
							segment._t1 = GeometryUtils.Map( (float)t1, 0, 1, p1._t1, p1._t2 );
							segment._t2 = GeometryUtils.Map( (float)t2, 0, 1, p1._t1, p1._t2 );
							pass2.Add( segment );
							t1 = t2;
							break;
						}
		...
		public Bezier Scale( Func<float, float> distanceFn )
		{
			...
			var o = GeometryUtils.Lli4( v0, c0, v1, c1 );
		...
		public Bezier Scale( float d )
		{
			...
			if ( order != 2 )
			{
				...
				np[1] = GeometryUtils.Lli4( p, p2, o.Value, points[1] ).Value;
			}

			{
				var p = np[order];
				var p2 = p + this.Tangent( 1 );
				np[1 + 1] = GeometryUtils.Lli4( p, p2, o.Value, points[1 + 1] ).Value;
			}
			return new Bezier( np );
		}
		...
		public float[] Intersects( Line line )
		{
			...
			return GeometryUtils.Roots( this.points, line ).Where( ( t ) =>
			{
				var p = self.Position( t );
				return GeometryUtils.Between( p.x, mx, MX ) && GeometryUtils.Between( p.y, my, MY );
			} ).ToArray();
		}
		...
		private static List<Pair<float>> Intersects( IEnumerable<Bezier> c1, IEnumerable<Bezier> c2, float curveIntersectionThreshold )
		{
			...
			{
				var result = GeometryUtils.PairIteration(
				  pair.Left,
				  pair.Right,
		...
		float _error( Vector2 pc, Vector2 np1, float s, float e )
		{
			...
			var @ref = Vector2.DistanceBetween( pc, np1 );
			var d1 = Vector2.DistanceBetween( pc, c1 );
			var d2 = Vector2.DistanceBetween( pc, c2 );
		...
		List<Arc> _iterate( float errorThreshold, List<Arc> circles )
		{
			...
			do
			{
				...
				do
				{
					...
					arc = GeometryUtils.Getccenter( np1, np2, np3 );
					...
					if ( curr_good )
					{
						...
						if ( t_e >= 1 )
						{
							...
							if ( t_e > 1 )
							{
								...
								arc.EndAngle += GeometryUtils.Angle( arc.Center, d, this.Position( 1 ) );
							}
							break;
						}
		...
		{
			var aligned = GeometryUtils.Align( points, new Line { P1 = start, P2 = end } );
		...
		{
			return GeometryUtils.Inflections( points );
		}
		...
		private static void CheckLinear( Bezier curve )
		{
			...
			Vector2[] a = GeometryUtils.Align( points, new Line { P1 = points[0], P2 = points[order] } ).ToArray();
		...
		private static float GetError( Bezier b, float startT, float endT, IPathShape arc, bool arcReversed )
		{
			...
			float m( float ratio )
			{
				...
				return  Vector2.DistanceBetween( ap, bp );
			}
