...
{
	static internal class GeometryUtils
	{
	  ...
	  0.0123412297999871995468056670700372915759f
		...
		public static void Closest( IEnumerable<Vector2> LUT, Vector2 point, out float mdist, out int mpos )
		{
			...
			{
				var d = Vector2.DistanceBetween( point, p );
				if ( d < mdist )
				{
					mdist = d;
					mpos = idx;
		...
		{
			return Roots( points, new Line { P1 = Vector2.Zero, P2 = Vector2.Right } );
		}
		...
		public static float[] Inflections( Vector2[] points )
		{
			...
			var p = GeometryUtils.Align( points, new Line { P1 = points[0], P2 = points.Last() } ).ToArray();
			...
			if ( GeometryUtils.Approximately( v1, 0 ) )
			{
				if ( !GeometryUtils.Approximately( v2, 0 ) )
				{
					var t = -v3 / v2;
			...
			if ( GeometryUtils.Approximately( dd, 0 ) ) return new float[0];
		...
		public static List<Pair<float>> PairIteration( Bezier c1, Bezier c2, float threshold )
		{
			...
			{
				results.AddRange( GeometryUtils.PairIteration( pair.Left, pair.Right, threshold ) );
			}
		...
		public static Arc Getccenter( Vector2 p1, Vector2 p2, Vector2 p3 )
		{
			...
			var arc = GeometryUtils.Lli8( mx1, my1, mx1n, my1n, mx2, my2, mx2n, my2n ).Value;
			var r = Vector2.DistanceBetween( arc, p1 );
		...
		public static Vector2 Vector2Min(Vector2 a, Vector2 b)
		{
		...
		public static Vector2 Vector2Max( Vector2 a, Vector2 b )
		{
			if ( a.Length < b.Length )
			{
				return b;
